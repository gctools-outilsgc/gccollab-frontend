...
export class DurationFormatPipe implements PipeTransform {
  ...
  transform(startDate: Date, endDate: Date): string {
    ...
    if (months > 0) {
      result +=
        months > 1 ? `${months} ${this.translate.instant(this.translations.duration.month.plural)}, ` : `${months} ${this.translate.instant(this.translations.duration.month.singular)}, `;
    }
    if (weeks > 0) {
    ...
    if (minutes > 0) {
      result +=
        minutes > 1 ? `${minutes} ${this.translate.instant(this.translations.duration.minute.plural)}, ` : `${minutes} ${this.translate.instant(this.translations.duration.minute.singular)}, `;
    }
