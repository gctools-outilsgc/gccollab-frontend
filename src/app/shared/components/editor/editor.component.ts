    ...
    ControlValueAccessor
{
  ...
  ngAfterViewInit() {
    ...
    this.focusChange = new MutationObserver((mutations: MutationRecord[]) => {
      mutations.forEach(() => {
        const classList =
          this.editorViewChild.nativeElement?.children[1]?.children[0]
            ?.children[0]?.classList;
        if (classList) {
          this.hasFocus = Array.from(classList).includes('ProseMirror-focused')
            ? true
            : false;
    ...
    const node =
      this.editorViewChild.nativeElement?.children[1]?.children[0]?.children[0];
  ...
  onLangChange(): void {
    this.ngxEditorService.config.locals = ngxEditorLocals(
      ...
      this.addAccessibility(
        '[title="' +
          this.translateService.instant(
            this.translations.editor.horizontalrule,
          ) +
          '"]',
  ...
  addAccessibility(selector: string, ariaLabel: string): void {
    const el: HTMLElement =
      this.elementRef.nativeElement.querySelectorAll(selector)[0];
    ...
    if (el) {
      ...
      el.setAttribute(
        'ariaLabel',
        el.classList.contains('NgxEditor__MenuItem--Active')
          ? ariaLabel +
              this.translateService.instant(this.translations.editor.enabled)
          : ariaLabel,
      );
  ...
  toggleAria(event: Event) {
    setTimeout(() => {
      if (event.target instanceof HTMLElement) {
        if (
          event.target.classList.contains('NgxEditor__MenuItem--Active') ||
          event.target.classList.contains('NgxEditor__Dropdown--Active')
        ) {
          event.target.setAttribute(
            'ariaLabel',
            this.cleanAriaLable(event.target) +
              this.translateService.instant(this.translations.editor.enabled),
          );
        } else {
          event.target.setAttribute(
